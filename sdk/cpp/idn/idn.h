...
//  10/2024 Gitle Mikkelsen, extended with more types of data frames, like high
//  res and 8 color channels
// -------------------------------------------------------------------------------------------------
...
// Many functions taken from idtfPlayer/main.cpp: Copyright (c) 2016 DexLogic,
// Dirk Apitz. MIT License
...
#define MAX_IDN_MESSAGE_LEN                                                    \
  (1454) // IDN-Message maximum length (1454 is MTU limit to avoid
         // fragmentation)
...
typedef struct {
  ...
  // unsigned usFrameTime;                 // Time for one frame in microseconds
  // (1000000/frameRate) int jitterFreeFlag;                   // Scan frames
  // only once to exactly match frame rate
  ...
  uint8_t
      *queuedBufferPtr; // Pointer to work buffer 2, acting as a double buffer
  uint8_t *controlBufferPtr; // Pointer to control buffer, for transactions
                             // other than frames
  ...
  uint64_t
      averageSleepError; // Debug info, sleep function inaccuracy statistics
  unsigned int
      packetNumFragments; // Multiple of MTU size to send UDP packets with.
...
int idnOpenFrameGeneric(IDNCONTEXT *context, uint16_t *channelDescriptors,
                        size_t numChannelDescriptors, bool forceNewConfig);
int idnPutSampleGeneric(IDNCONTEXT *context, int8_t *sampleBuffer,
                        size_t sampleBufferSize);
int idnOpenFrameXYRGB(IDNCONTEXT *context, bool forceNewConfig);
int idnPutSampleXYRGB(IDNCONTEXT *context, int16_t x, int16_t y, uint8_t r,
                      uint8_t g, uint8_t b);
int idnOpenFrameXYRGBI(IDNCONTEXT *context, bool forceNewConfig);
int idnPutSampleXYRGBI(IDNCONTEXT *context, int16_t x, int16_t y, uint8_t r,
                       uint8_t g, uint8_t b, uint8_t i);
int idnOpenFrameHighResXYRGB(IDNCONTEXT *context, bool forceNewConfig);
int idnPutSampleHighResXYRGB(IDNCONTEXT *context, int16_t x, int16_t y,
                             uint16_t r, uint16_t g, uint16_t b);
int idnOpenFrameExtended(IDNCONTEXT *context, bool forceNewConfig);
int idnPutSampleExtended(IDNCONTEXT *context, int16_t x, int16_t y, uint16_t r,
                         uint16_t g, uint16_t b, uint16_t i, uint16_t u1,
                         uint16_t u2, uint16_t u3, uint16_t u4);
