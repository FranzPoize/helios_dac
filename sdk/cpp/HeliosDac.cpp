...
HeliosDac::HeliosDac() { inited = false; }
...
int HeliosDac::OpenDevicesOnlyUsb() {
  ...
  // Validate monotonic time reference. Done in IDN but must be done here if not
  // using IDN
...
int HeliosDac::ReScanDevicesOnlyUsb() {
  if (!inited) {
    // Validate monotonic time reference. Done in IDN but must be done here if
    // not using IDN
    if (plt_validateMonoTime() != 0) {
      logError("Monotonic time init failed");
      return 0;
...
// Internal function. inPlace = Whether to keep the current opened devices in
// their device number slots and only scan for changes.
int HeliosDac::_OpenUsbDevices(bool inPlace) {
  ...
  if (inPlace && inited) {
    // Verify connection of existing devices, close them if they cannot be
    // reached
  ...
  for (int i = 0; i < cnt; i++) {
      ...
      for (int j = 0; j < deviceList.size(); j++) {
        ...
        if (!deviceList[j]->GetIsClosed()) {
          libusb_device_handle *handle =
              ((HeliosDacUsbDevice *)(deviceList[j].get()))->GetLibusbHandle();
          if (handle == NULL)
            continue;
          uint8_t newPortNumbers[7], existingPortNumbers[7];
          int existingPortNumberDepth = libusb_get_port_numbers(
              libusb_get_device(handle), existingPortNumbers, 16);
          int newPortNumberDepth =
              libusb_get_port_numbers(devs[i], newPortNumbers, 16);
    ...
    std::unique_ptr<HeliosDacDevice> device =
        std::make_unique<HeliosDacUsbDevice>(devHandle);

    if (inPlace) {
      // Check if this is an already known device that was previously closed, if
      // so insert it back to its previous device number
      bool found = false;
      for (int j = 0; j < deviceList.size(); j++) {
        if (!deviceList[j]->GetIsUsb())
          continue;
...
void HeliosDac::_RemoveNotFoundIdnServers(IDNSL_SERVER_INFO *firstServerInfo) {
  for (int j = 0; j < deviceList.size(); j++) {
    ...
    bool found = false;
    for (IDNSL_SERVER_INFO *serverInfo = firstServerInfo; serverInfo != 0;
...
// Internal helper function, returns true if device with this unit ID already
// exists in device list
bool HeliosDac::_GetIdnServerExists(IDNSL_SERVER_INFO *serverInfo) {
  for (int j = 0; j < deviceList.size(); j++) {
    ...
    ((HeliosDacIdnDevice *)deviceList[j].get())->GetUnitId(id);
    bool unitIdMismatch =
        memcmp(serverInfo->unitID, id, IDNSL_UNITID_LENGTH) != 0;
    if (!unitIdMismatch) {
      return true;
    }
  }
  ...
  if (rcGetList != 0) {
    ...
    for (IDNSL_SERVER_INFO *serverInfo = firstServerInfo; serverInfo != 0;
      ...
      for (unsigned int i = 0; i < serverInfo->addressCount; i++) {
        if (serverInfo->addressTable[i].errorFlags == 0 &&
            idnContexts.size() < 999) {
          bool found = false;
          // Check for duplicate entries from scan
          for (long unsigned int contextId = 0; contextId < idnContexts.size();
               contextId++) {
            bool unitIdMismatch =
                memcmp(serverInfo->unitID, idnContexts[contextId]->unitId,
                       IDNSL_UNITID_LENGTH) != 0;

            if (idnContexts[contextId]->serverSockAddr.sin_addr.s_addr ==
                    serverInfo->addressTable[i].addr.s_addr ||
                !unitIdMismatch)
              found = true;
          }
          if (inPlace && !found) {
    ...
    freeIDNServerList(firstServerInfo); // Server list is dynamically allocated
                                        // and must be freed
  }
#else
  // Windows
      ...
      for (unsigned int i = 0; i < serverInfo->addressCount; i++) {
        if (serverInfo->addressTable[i].errorFlags == 0 &&
          ...
          for (int contextId = 0; contextId < idnContexts.size(); contextId++) {
            ...
            if (idnContexts[contextId]->serverSockAddr.sin_addr.S_un.S_addr ==
                    serverInfo->addressTable[i].addr.S_un.S_addr ||
                !unitIdMismatch) {
              found = true;
              break;
            }
...
int HeliosDac::WriteFrame(unsigned int devNum, unsigned int pps,
                          std::uint8_t flags, HeliosPoint *points,
...
int HeliosDac::WriteFrameHighResolution(unsigned int devNum, unsigned int pps,
                                        unsigned int flags,
...
int HeliosDac::WriteFrameExtended(unsigned int devNum, unsigned int pps,
...
HeliosDac::HeliosDacUsbDevice::HeliosDacUsbDevice(
  ...
  while (libusb_interrupt_transfer(usbHandle, EP_INT_IN, ctrlBuffer0, 32,
                                   &actualLength, 5) == LIBUSB_SUCCESS)
    ;
  ...
  for (int i = 0; ((i < 2) && repeat); i++) // retry command if necessary
  {
    ...
    int transferResult = libusb_interrupt_transfer(
        usbHandle, EP_INT_OUT, ctrlBuffer, 2, &actualLength, 32);
    if ((transferResult == LIBUSB_SUCCESS) && (actualLength == 2)) {
      for (int j = 0; ((j < 3) && repeat);
           j++) // retry response getting if necessary
      {
        std::uint8_t ctrlBuffer2[32];
        transferResult = libusb_interrupt_transfer(
            usbHandle, EP_INT_IN, ctrlBuffer2, 32, &actualLength, 32);
        if (transferResult == LIBUSB_SUCCESS) {
          if (ctrlBuffer2[0] == 0x84) {
            firmwareVersion = ((ctrlBuffer2[1] << 0) | (ctrlBuffer2[2] << 8) |
                               (ctrlBuffer2[3] << 16) | (ctrlBuffer2[4] << 24));
            repeat = false;
          }
        }
      }
  ...
  for (int i = 0; ((i < 2) && repeat); i++) // retry command if necessary
  {
    ...
    int transferResult = libusb_interrupt_transfer(
        usbHandle, EP_INT_OUT, ctrlBuffer3, 2, &actualLength, 32);
    if ((transferResult == LIBUSB_SUCCESS) && (actualLength == 2))
      repeat = false;
  }
  ...
  std::thread frameHandlerThread(
      &HeliosDac::HeliosDacUsbDevice::BackgroundFrameHandler, this);
  frameHandlerThread.detach();
}

// Sends a raw frame buffer (implemented as bulk transfer) to a dac device
// Returns 1 if success
int HeliosDac::HeliosDacUsbDevice::SendFrame(unsigned int pps,
                                             std::uint8_t flags,
                                             HeliosPoint *points,
                                             unsigned int numOfPoints) {
  if (GetIsClosed())
  ...
  // If pps is too low, try to duplicate frames to simulate a higher pps rather
  // than failing
  ...
  if (pps < GetMinSampleRate()) {
    ...
    HeliosPoint *duplicatedPoints =
        new HeliosPoint[numOfPoints * samplingFactor];
    freePoints = true;
  ...
  // If pps is too high or frame size too high, subsample frames to simulate a
  // lower pps/size rather than failing
  ...
  // This is a bug workaround, the mcu won't correctly receive transfers with
  // these sizes
  ...
  if ((((int)numOfPoints - 45) % 64) == 0) {
    ...
    ppsActual =
        (unsigned int)((pps * (double)numOfPointsActual / (double)numOfPoints) +
                       0.5);
  }
...
// Sends a raw frame buffer (implemented as bulk transfer) to a dac device,
// using high-res point structure (but simply convert down to low-res) Returns 1
// if success
int HeliosDac::HeliosDacUsbDevice::SendFrameHighResolution(
    unsigned int pps, std::uint8_t flags, HeliosPointHighRes *points,
    unsigned int numOfPoints) {
  if (GetIsClosed())
    return HELIOS_ERROR_DEVICE_CLOSED;
  ...
  if (pps < GetMinSampleRate()) {
    ...
    HeliosPointHighRes *duplicatedPoints =
        new HeliosPointHighRes[numOfPoints * samplingFactor];
    freePoints = true;
...
// Sends a raw frame buffer (implemented as bulk transfer) to a dac device,
// using extended point structure (but simply convert down to low-res) Returns 1
// if success
int HeliosDac::HeliosDacUsbDevice::SendFrameExtended(unsigned int pps,
                                                     std::uint8_t flags,
  ...
  if (pps < GetMinSampleRate()) {
    ...
    HeliosPointExt *duplicatedPoints =
        new HeliosPointExt[numOfPoints * samplingFactor];
    freePoints = true;
...
int HeliosDac::HeliosDacUsbDevice::DoFrame() {
  ...
  int transferResult =
      libusb_bulk_transfer(usbHandle, EP_BULK_OUT, frameBuffer, frameBufferSize,
                           &actualLength, 8 + (frameBufferSize >> 5));

  // auto now = std::chrono::high_resolution_clock::now();
  // auto time = std::chrono::duration_cast<std::chrono::milliseconds>(now -
  // then); printf("%d : Sent bulk frame, %d, time %d\n",
  // std::chrono::duration_cast<std::chrono::milliseconds>(then.time_since_epoch()),
  // transferResult, time.count());
...
int HeliosDac::HeliosDacUsbDevice::GetName(char *dacName) {
  ...
  for (int i = 0; i < 2; i++) // retry command if necessary
  {
    ...
    if (SendControl(ctrlBuffer4, 2) == HELIOS_SUCCESS) {
      ...
      int transferResult =
          libusb_interrupt_transfer(usbHandle, EP_INT_IN, ctrlBuffer5,
                                    sizeof(ctrlBuffer5), &actualLength, 32);
...
int HeliosDac::HeliosDacUsbDevice::GetStatus() {
  ...
  if (SendControl(ctrlBuffer, 2) == HELIOS_SUCCESS) {
    std::uint8_t ctrlBuffer2[32];
    int transferResult = libusb_interrupt_transfer(
        usbHandle, EP_INT_IN, ctrlBuffer2, 32, &actualLength, 16);

    // auto now = std::chrono::high_resolution_clock::now();
    // auto time = std::chrono::duration_cast<std::chrono::milliseconds>(now -
    // then); printf("%d : Sent status req, %d, time %d, status %d\n",
    // std::chrono::duration_cast<std::chrono::milliseconds>(then.time_since_epoch()),
    // transferResult, time.count(), ctrlBuffer2[1]);
...
bool HeliosDac::HeliosDacUsbDevice::GetDidSendFrameRecently() {
  if (GetIsClosed())
    return false;
  return plt_getMonoTimeUS() - lastSendTime <
         500000; // less than 500 ms since last frame send
}
...
// sends a raw control signal (implemented as interrupt transfer) to a dac
// device returns 1 if successful
int HeliosDac::HeliosDacUsbDevice::SendControl(std::uint8_t *bufferAddress,
                                               unsigned int length) {
  ...
  int actualLength = 0;
  int transferResult = libusb_interrupt_transfer(
      usbHandle, EP_INT_OUT, bufferAddress, length, &actualLength, 16);
...
HeliosDac::HeliosDacUsbDevice::~HeliosDacUsbDevice() {
  ...
  std::lock_guard<std::mutex> lock(
      frameLock); // wait until all threads have closed
...
HeliosDac::HeliosDacIdnDevice::HeliosDacIdnDevice(IDNCONTEXT *_context) {
  ...
  if (context->fdSocket < 0) {
    logError("socket() error when initing IDN device connection: %d",
             plt_sockGetLastError());

    return;
  }

  // Ensure UDP socket buffer size is big enough. No longer required after
  // switching to wave mode.
  /*#if defined(_WIN32) || defined(WIN32)
          DWORD bufferSize = 0xFFFF;
          int res = setsockopt(context->fdSocket, SOL_SOCKET, SO_SNDBUF, (const
  char*)&bufferSize, sizeof(bufferSize)); #else int bufferSize = 0xFFFF; int res
  = setsockopt(context->fdSocket, SOL_SOCKET, SO_SNDBUF, (const
  void*)&bufferSize, sizeof(bufferSize)); #endif if (res != 0)
          {
                  logError("Error setting max UDP buffer size for IDN device:
  %d", plt_sockGetLastError());
                  //return;
          }
          */
  ...
  if (managementSocket < 0) {
    fprintf(stderr, "managementSocket open failed (error: %d)",
            plt_sockGetLastError());
  }

#ifdef WIN32
  DWORD ms = 700;
  setsockopt(managementSocket, SOL_SOCKET, SO_RCVTIMEO, (const char *)&ms,
             sizeof(ms));
#else
  struct timeval tv;
  tv.tv_sec = 0;
  tv.tv_usec = 700000;
  setsockopt(managementSocket, SOL_SOCKET, SO_RCVTIMEO, (const void *)&tv,
             sizeof(tv));
#endif
  ...
  std::thread frameHandlerThread(
      &HeliosDac::HeliosDacIdnDevice::BackgroundFrameHandler, this);
  frameHandlerThread.detach();
}

// Queues up a raw frame buffer to be sent to the DAC. Legacy low-res point
// format. Returns 1 if success
int HeliosDac::HeliosDacIdnDevice::SendFrame(unsigned int pps,
                                             std::uint8_t flags,
                                             HeliosPoint *points,
                                             unsigned int numOfPoints) {
  if (GetIsClosed())
  ...
  // If pps is too low or frame too small, try to duplicate points to simulate a
  // larger, slower frame rather than failing
  bool freePoints = false;
  ...
  // lower pps/size rather than failing
  unsigned int samplingFactor = 1;
  if (pps > GetMaxSampleRate() ||
      numOfPoints > GetMaxFrameSize(XYRGBI_SAMPLE_SIZE)) {
    samplingFactor = pps / GetMaxSampleRate() + 1;
    if ((numOfPoints / GetMaxFrameSize(XYRGBI_SAMPLE_SIZE) + 1) >
        samplingFactor)
  ...
  // Applies to IDN frame mode, but not wave mode.

  unsigned int loopLength = numOfPoints * samplingFactor;
  for (unsigned int i = 0; i < loopLength; i += samplingFactor) {
    if (idnPutSampleXYRGBI(context, (points[i].x << 4) - 0x8000,
                           (points[i].y << 4) - 0x8000, points[i].r,
                           points[i].g, points[i].b, points[i].i))
      return false;
  }
...
// Queues up a raw frame buffer to be sent to the DAC. Modern high-res point
// format. Returns 1 if success
int HeliosDac::HeliosDacIdnDevice::SendFrameHighResolution(
    unsigned int pps, std::uint8_t flags, HeliosPointHighRes *points,
    unsigned int numOfPoints) {
  if (GetIsClosed())
    return HELIOS_ERROR_DEVICE_CLOSED;
  ...
  if (pps < GetMinSampleRate() || numOfPoints < GetMinFrameSize()) {
    ...
    if (numOfPoints * samplingFactor >
        GetMaxFrameSize(XYRGB_HIGHRES_SAMPLE_SIZE))
      return HELIOS_ERROR_PPS_TOO_LOW;
    else if (pps * samplingFactor > GetMaxSampleRate())
      return HELIOS_ERROR_FRAME_TOO_SMALL;
  ...
  unsigned int samplingFactor = 1;
  if (pps > GetMaxSampleRate() ||
      numOfPoints > GetMaxFrameSize(XYRGB_HIGHRES_SAMPLE_SIZE)) {
    samplingFactor = pps / GetMaxSampleRate() + 1;
    if ((numOfPoints / GetMaxFrameSize(XYRGB_HIGHRES_SAMPLE_SIZE) + 1) >
        samplingFactor)
      samplingFactor =
          numOfPoints / GetMaxFrameSize(XYRGB_HIGHRES_SAMPLE_SIZE) + 1;
  ...
  unsigned int loopLength = numOfPoints * samplingFactor;
  for (unsigned int i = 0; i < loopLength; i += samplingFactor) {
    if (idnPutSampleHighResXYRGB(context, points[i].x - 0x8000,
                                 points[i].y - 0x8000, points[i].r, points[i].g,
                                 points[i].b))
      return false;
  }
...
// Queues up a raw frame buffer to be sent to the DAC. Extended point format
// with optional user1-4 channels. Returns 1 if success
int HeliosDac::HeliosDacIdnDevice::SendFrameExtended(unsigned int pps,
                                                     std::uint8_t flags,
                                                     HeliosPointExt *points,
                                                     unsigned int numOfPoints) {
  if (GetIsClosed())
  ...
  unsigned int samplingFactor = 1;
  if (pps > GetMaxSampleRate() ||
      numOfPoints > GetMaxFrameSize(EXTENDED_SAMPLE_SIZE)) {
    samplingFactor = pps / GetMaxSampleRate() + 1;
    if ((numOfPoints / GetMaxFrameSize(EXTENDED_SAMPLE_SIZE) + 1) >
        samplingFactor)
  ...
  unsigned int loopLength = numOfPoints * samplingFactor;
  for (unsigned int i = 0; i < loopLength; i += samplingFactor) {
    if (idnPutSampleExtended(context, (int16_t)(points[i].x - 0x8000),
                             (int16_t)(points[i].y - 0x8000), points[i].r,
                             points[i].g, points[i].b, points[i].i,
                             points[i].user1, points[i].user2, points[i].user3,
                             points[i].user4))
      return false;
  }
...
int HeliosDac::HeliosDacIdnDevice::GetStatus() {
  ...
  if (context->frameReady) // || (plt_getMonoTimeUS() < (statusReadyTime -
                           // bufferTimeMs * 1000)))
  {
    // Simulate a small delay to mimic behavior of USB device, for backwards
    // compatibility
    if (!useBusyWaiting)
      plt_usleep(50);
    else {
...
// Sends wave packet to DAC. Needs to be called periodically with good timing,
// as close to, but not earlier than, context->frameTimestamp.
...
// Continually running thread. Waits for a new packet to be ready, then it is
// sent to the DAC at the correct time.
void HeliosDac::HeliosDacIdnDevice::BackgroundFrameHandler() {
  while (!GetIsClosed()) {
    ...
    if (!context->isStoppedOrTimeout) {
      if (timeLeft == 0) {
        ...
        if (numLateWaits > 10 && context->packetNumFragments < 6) {
          context->packetNumFragments++; // Increase max UDP packet size to have
                                         // better sleep time error margins.
          printf("IDN - NB: Increased max UDP packet size multiplier to %d to "
                 "increase sleep error margin.\n",
    ...
    if (context->isStoppedOrTimeout) {
      ...
      // We add a bit of artificial jitter to wait period. This makes certain
      // IDN server implementations such as StageMate have a more lenient buffer
      // duration. To get even better latency at the expense of stability, you
      // can remove the added random part of the delay. Or if you want the
      // absolute lowest jitter and thus latency possible, you can set
      // useBusyWaiting to true, but this ravages the CPU usage.
    } else {
    ...
    if (!context->isStoppedOrTimeout) {
      ...
      context->averageSleepError =
          (context->averageSleepError * NUM_SLEEP_ERROR_SAMPLES + sleepError) /
          (NUM_SLEEP_ERROR_SAMPLES + 1);
      if ((debugMessageCount++ % 1000) == 0)
        printf("IDN timing: Time now: %d, left: %d, sleep err: %d\n", now,
               timeLeft, context->averageSleepError);
    }
#endif
...
int HeliosDac::HeliosDacIdnDevice::GetFirmwareVersion() {
  ...
  int sentBytes = 0;
  sentBytes = sendto(managementSocket, buffer, 2, 0,
                     (const sockaddr *)&managementSocketAddr,
                     sizeof(managementSocketAddr));
  ...
  int numBytes = recvfrom(managementSocket, buffer, sizeof(buffer), 0,
                          &responseAddr, &responseAddrLength);
  if (numBytes > 3) {
...
int HeliosDac::HeliosDacIdnDevice::SetName(char *name) {
  ...
  buffer[22] = '\0'; // Safety
  int sentBytes = 0;
  sentBytes = sendto(managementSocket, buffer, 22, 0,
                     (const sockaddr *)&managementSocketAddr,
                     sizeof(managementSocketAddr));
...
bool HeliosDac::HeliosDacIdnDevice::GetDidSendFrameRecently() {
  if (GetIsClosed())
    return false;
  return plt_getMonoTimeUS() - context->frameTimestamp <
         500000; // 500 ms since last frame send
}
